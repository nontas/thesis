...
that we have estimated the optimal appearance parameters
...
\end{equation}
%%%%%%%%%%%%%%
we compute the optimal block-diagonal affinity matrix (graph) by solving the
...
%%%%%%%%%%%%%%
\begin{equation}
  \mathbf{Z} = \left( \mathbf{A}^{\mathsf{T}}\mathbf{A} + \mu \mathbf{I}_{eye} \right)^{-1} \mathbf{A}^{\mathsf{T}}\mathbf{A}
\end{equation}
%%%%%%%%%%%%%%
