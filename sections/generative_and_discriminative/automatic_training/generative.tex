...
optimization problem that aims to automatically construct an optimal generative
...
&\text{subject to} & & \mathbf{U}_a^{\mathsf{T}}\mathbf{U}_a=\mathbf{I}_{eye}
\end{aligned}
\label{equ:problem}
\end{equation}
%%%%%%%%%%%%%%
in order to find the optimal appearance subspace $\mathbf{U}_a$ and mean vector
...
the training image $\mathbf{t}^i$ and $\mathbf{I}_{eye}$ denotes the identity
...
$\mathbf{U}_a^{\mathsf{T}}\mathbf{U}_a=\mathbf{I}_{eye}$. In this work, we
...
estimate of $\mathbf{p}^i$ and compute the optimal appearance parameters as the
